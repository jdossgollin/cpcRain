...
```{r}
dt1 <- cpcQueryDateRange(
  start_date = ymd('1998-12-24'),
...
**Because this takes a very long time, the following code chunks are not run!**

```{r, eval = FALSE}
download_years <- 1979:1980
...
In that case, you'll notice that the units on the `time` variable are a bit odd:
...
However, the advantage here is that the `lubridate` package's `as_date` function takes the origin to be 1970-01-01 by default, so you can call `lubridate::as_date(nc$dim$time$vals)` without any trouble.
